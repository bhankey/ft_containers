...
#ifndef FT_CONTAINERS_CONTAINERS_MULTIMAP_HPP
#define FT_CONTAINERS_CONTAINERS_MULTIMAP_HPP
...
public:
  ...
  Multimap(const Compare &comp = Compare(), const allocator_type &allocator = Allocator()): rb_tree_(comp, allocator), compare_(comp) {
  }
  template<typename InputIt>
  Multimap(InputIt first, InputIt last, const Compare &comp = Compare(), const Allocator allocator = Allocator()): rb_tree_(comp, allocator), compare_(comp) {
    while (first != last) {
      rb_tree_.insert_equal(*first);
      ++first;
    }
  }
  Multimap(const Multimap &other): rb_tree_(other.rb_tree_), compare_(other.compare_) {
  }
...
bool operator==(const Multimap<Key, T, Compare, Alloc> &lhs, const Multimap<Key, T, Compare, Alloc> &rhs) {
  ...
  typename ft::Multimap<Key,T, Compare, Alloc>::const_iterator lit = lhs.begin(), rit = rhs.begin();
...
#endif //FT_CONTAINERS_CONTAINERS_MULTIMAP_HPP
